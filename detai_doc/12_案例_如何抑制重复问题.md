...
**State + Planner + Retrieval + Validator + Re-plan 联合问题**
...
### 第五步：命中重复后触发“重规划”而不是直接失败

对应本项目：

- `app/graphs/interview_nodes.py` 中 `generate_question_for_state()`

当前实现不是“判重后直接报错”，而是：

1. 先用 `is_question_too_similar()` 判断草稿是否与旧问题过近。
2. 若命中重复，构造 `blocked_branch_ids` 与 `blocked_target_signatures`。
3. 重新调用 `plan_next_question()` 选择不同 target，再二次生成问题。

所以重复治理在这里形成了一个“检测 -> 重规划 -> 再生成”的闭环。
...
```text
question_history + planner 去重 + retrieval novelty penalty + validator + 命中重复后重规划 + 可选 embedding
```
