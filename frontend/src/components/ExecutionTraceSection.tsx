...
function StepRow({ step, isCurrent }: { step: RunStepRead; isCurrent: boolean }) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[0.72rem] uppercase tracking-[0.16em] opacity-80">
            ...
            {step.next_step_hint ? <span>Next: {step.next_step_hint}</span> : null}
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] opacity-75">Duration</p>
          {step.status === 'running' ? (
            <PretextElapsedTime
            ...
            <p className="mt-1 text-sm font-semibold">{formatDurationMs(step.duration_ms)}</p>
          )}
          {step.total_tokens > 0 ? (
            <p className="mt-1 text-xs opacity-75">{step.total_tokens} tokens</p>
          ) : null}
        </div>
      </div>
    </div>
...
export const ExecutionTraceSection = memo(function ExecutionTraceSection({
  ...
  const runStatusLabel =
    run.status === 'running' ? 'In progress' : run.status === 'failed' ? 'Failed' : 'Completed'
      ...
      >
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-slate-700">
            ...
            <span>{completedSteps}/{Math.max(run.step_count, run.steps.length)} steps</span>
            {currentStep ? (
              <>
                <span className="text-slate-400">•</span>
                {run.status === 'running' ? (
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            {run.status === 'running' ? (
              <PretextElapsedTime className="mt-1 block text-sm font-semibold text-slate-950" startedAt={run.started_at} />
            ) : (
              <p className="mt-1 text-sm font-semibold text-slate-950">{formatDurationMs(run.duration_ms)}</p>
            )}
          </div>
    ...
    <div className="rounded-[1.75rem] border border-amber-200 bg-[linear-gradient(180deg,rgba(255,251,235,0.96),rgba(255,255,255,0.98))] p-5 shadow-[0_18px_40px_rgba(251,191,36,0.12)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-3 font-serif text-2xl text-slate-950">Generating the next question</h3>
          <p className="mt-2 text-sm leading-7 text-slate-700">
            The agent is moving through the orchestration steps below. This panel updates while the run is active.
          </p>
        </div>
      ...
      <div className="mt-4">
        <ExecutionTraceSection defaultExpanded run={run} title="Current Run" />
      </div>
    </div>
  )
})
